...
import { ChainId, CurrencyAmount, Price, Token, WETH9 } from '@ring-protocol/sdk-core'
...
describe('computePairAddress', () => {
  it('should correctly compute the pool address', () => {
    ...
    const result = computePairAddress({
      initCodeHash: INIT_CODE_HASH_MAP[ChainId.MAINNET],
      factoryAddress: '0x1111111111111111111111111111111111111111',
      tokenA,
      tokenB,
    })

    expect(result).toEqual('0x7071972A3bD385A204F64e02f161C067EDf4926B')
  })
  ...
  describe('#getAddress', () => {
    it('returns the correct address', () => {
      expect(Pair.getAddress(USDC, DAI)).toEqual(
        computePairAddress({
          initCodeHash: INIT_CODE_HASH_MAP[ChainId.MAINNET],
          factoryAddress: FACTORY_ADDRESS_MAP[ChainId.MAINNET],
          tokenA: USDC,
          tokenB: DAI,
        })
      )
    ...
    it('returns the chain-specific mapped address for Sepolia', () => {
      expect(Pair.getAddress(USDC_SEPOLIA, DAI_SEPOLIA)).toEqual(
        computePairAddress({
          initCodeHash: INIT_CODE_HASH_MAP[ChainId.SEPOLIA],
          factoryAddress: FACTORY_ADDRESS_MAP[ChainId.SEPOLIA],
          tokenA: USDC_SEPOLIA,
          tokenB: DAI_SEPOLIA,
        })
      )
