...
import { getFewTokenFromOriginalToken, isFewToken } from '../fewToken'
...
export class Trade<TInput extends Currency, TOutput extends Currency, TTradeType extends TradeType> {
  ...
  public constructor(
    ...
    const chainId: number = route.pairs[0].chainId

    if (tradeType === TradeType.EXACT_INPUT) {
      if (
        isFewToken(route.pairs[0].token0) ||
        isFewToken(route.pairs[0].token1) ||
        isFewToken(route.input.wrapped) ||
        isFewToken(route.output.wrapped)
      ) {
      ...
      if (
        ...
        tokenAmounts[tokenAmounts.length - 1] = CurrencyAmount.fromFractionalAmount(
          fewAmountToken,
          amount.numerator,
          amount.denominator
        )
